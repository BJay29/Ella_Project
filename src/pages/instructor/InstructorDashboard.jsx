        ...
        <div className="bg-white w-full px-8 py-4 flex justify-between items-center border-b border-gray-100 relative z-50">
            ...
            <div className="relative">
                <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="flex items-center gap-3 hover:bg-gray-50 p-1 pr-3 rounded-full transition-all group">
                    ...
                    <div className="h-10 w-10 bg-orange-500 rounded-full flex items-center justify-center text-white font-bold text-xs shadow-lg group-hover:scale-105 transition-transform">PG</div>
                </button>
                {isMenuOpen && (
                    <>
                        ...
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-50 py-2 z-20 animate-in slide-in-from-top-2 duration-200">
                            <button className="w-full px-4 py-3 text-left text-[10px] font-black text-gray-600 uppercase tracking-widest hover:bg-gray-50">👤 Profile Settings</button>
                            <div className="h-[1px] bg-gray-100 my-1 mx-4"></div>
                            <button onClick={() => { setIsMenuOpen(false); onLogoutClick(); }} className="w-full px-4 py-3 text-left text-[10px] font-black text-red-500 uppercase tracking-widest hover:bg-red-50">🚪 Logout</button>
                        </div>
                    </>
                )}
            </div>
...
const NavTab = ({ active, label, onClick, icon }) => (
    <button onClick={onClick} className={`flex items-center gap-2 pb-3 text-[10px] font-black tracking-widest transition-all relative group ${active ? 'text-[#22C55E]' : 'text-gray-400 hover:text-gray-600'}`}>
        <span className={`${active ? 'opacity-100' : 'opacity-50 group-hover:opacity-100'}`}>{icon}</span>
        {label}
        {active && <div className="absolute -bottom-[1px] left-0 w-full h-[3px] bg-[#22C55E] rounded-full animate-in slide-in-from-left-2 duration-300" />}
    </button>
);
...
const InstructorDashboard = () => {
    ...
    const stats = useMemo(() => ({
        pendingAlerts: 0, resolved: 0, totalStudents: 3, speakingPending: 0
    }), []);
    ...
    return (
        <div className="min-h-screen bg-[#F3F4F6] font-sans pb-10">
            <LogoutModal isOpen={isLogoutModalOpen} onClose={() => setLogout(false)} onConfirm={handleLogoutConfirm} />

            <TopNav onLogoutClick={() => setLogout(true)} />

            <div className="px-4 md:px-8 mt-6">
                <div className="bg-[#F59E0B] w-full p-8 rounded-[2rem] shadow-xl flex items-center gap-6">
                    <div className="bg-white/20 p-4 rounded-2xl backdrop-blur-md hidden sm:block">
                        <span className="text-2xl">📋</span>
                    </div>
                    <div>
                        <h1 className="text-white font-black italic uppercase text-2xl leading-none tracking-tight">Instructor Dashboard</h1>
                        <p className="text-white/80 text-[11px] font-bold mt-2 uppercase tracking-widest">Review student interventions and monitor performance</p>
                    </div>
                </div>
            </div>
                ...
                <main className="max-w-[1600px] mx-auto">
                    ...
                    {activeTab !== 'courses' && (
                        <div className="bg-white rounded-[3rem] p-20 text-center shadow-sm border border-gray-50 animate-in fade-in zoom-in-95">
                            ...
                            <p className="text-gray-400 text-[10px] font-bold mt-2 uppercase tracking-widest">Select another tab or check back later.</p>
                        </div>
                    )}
                </main>
