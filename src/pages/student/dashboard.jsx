...
const StudentDashboard = () => {
  ...
  // ✅ Fixed: clear localStorage on logout so token doesn't persist
  const handleLogout = () => {
    localStorage.clear();
      ...
      <main className="flex-1 flex flex-col p-4 md:p-10 overflow-hidden w-full">
        ...
        {activePage === 'Home' && (
          ...
          w-full flex flex-col overflow-hidden h-full">
            <div className="bg-[#2E7D32] border border-black text-white py-1 px-8 md:px-12 rounded-full w-max mx-auto mb-4 md:mb-6 text-[10px] md:text-xs font-bold uppercase tracking-widest flex-shrink-0">
              Quest
            </div>
                ...
                <div key={quest.id} className="border-b border-black/10 last:border-0">
                  ...
                  {selectedQuest === quest.id && (
                    <div className="flex justify-center pb-3 md:pb-4 pt-1 animate-fadeIn">
                      <button 
                        className="bg-[#2E7D32] text-white px-6 md:px-8 py-1.5 rounded-lg border border-black text-xs md:text-sm 
                        font-bold hover:bg-[#1B5E20] shadow-sm transition-all" 
