...
const SectionForm = ({ courseId, onNext }) => {
    // --- STATES ---
    ...
    const [generatedCode, setGeneratedCode] = useState(null); // Para sa bagong gawang section code

    // Form State base sa iyong requirement
    ...
    const fetchSections = async () => {
        ...
        try {
            ...
            // Ipinapasa ang courseId para makuha lang ang sections ng subject na iyon
            const response = await authAPI.getSectionsByCourse(courseId, token);
            if (response.ok) {
    ...
    const handleSaveSection = async (e) => {
        ...
        try {
            ...
            const response = editingId 
                ? await authAPI.updateSection(editingId, payload, token)
                : await authAPI.createSection(payload, token);

            if (response.ok) {
                const result = await response.json();
                
                // Kung bagong create, i-set ang generated code para ipakita sa user
    ...
    const handleDelete = async (e, id) => {
        ...
        if(window.confirm("Are you sure you want to delete this section?")) {
            try {
                const token = localStorage.getItem('token');
                const response = await authAPI.deleteSection(id, token);
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {loading && sections.length === 0 ? (
                <div className="py-24 text-center font-black text-gray-200 italic animate-pulse">LOADING SECTIONS...</div>
                ...
                <div className="flex flex-col items-center justify-center py-24 bg-white border-2 border-dashed border-gray-100 rounded-[3rem]">
                    ...
                    <p className="text-gray-400 text-sm mt-1">Mag-add ng section para makapagsimula.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {filteredSections.map((section) => (
                        <div 
                            key={section.id}
                            onClick={() => onNext(section.id, section.section_name)}
                        ...
                        >
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                <button onClick={(e) => openEditModal(e, section)} className="text-[9px] font-black uppercase text-amber-500 bg-amber-50 px-2 py-1 rounded-lg hover:bg-amber-500 hover:text-white transition-all">Edit</button>
                                <button onClick={(e) => handleDelete(e, section.id)} className="text-[9px] font-black uppercase text-red-500 bg-red-50 px-2 py-1 rounded-lg hover:bg-red-500 hover:text-white transition-all">Del</button>
                            </div>

                            <div className="w-14 h-14 bg-gray-50 rounded-full flex items-center justify-center text-xl mx-auto mb-4 group-hover:bg-indigo-600 group-hover:text-white transition-all">🏫</div>
                            ...
                            <div className="mt-2 space-y-1">
                                <p className="text-[10px] font-black text-indigo-500 bg-indigo-50 inline-block px-3 py-1 rounded-full uppercase">
                        ...
                        {!generatedCode ? (
                            <>
                                <div className="bg-indigo-600 p-6 text-white text-center">
                                    ...
                                    <p className="text-indigo-100 text-[10px] font-bold uppercase tracking-widest mt-1">Class Assignment</p>
                                </div>
                                ...
                                <form onSubmit={handleSaveSection} className="p-8 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">School Year</label>
                                            <input 
                                                required
                                                type="text"
                                                className="w-full mt-1.5 p-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-indigo-500 transition-all"
                                    ...
                                    <div className="flex gap-3 pt-4">
                                        <button type="button" onClick={closeModal} className="flex-1 py-4 bg-gray-100 hover:bg-gray-200 text-gray-500 font-black rounded-2xl text-[10px] uppercase tracking-widest transition-all">Cancel</button>
                                        <button type="submit" disabled={loading} className="flex-1 py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-lg transition-all active:scale-95 disabled:opacity-50">
                            ...
                            <div className="p-10 text-center animate-in fade-in zoom-in duration-300">
                                ...
                                <h3 className="text-2xl font-black text-gray-800 uppercase italic tracking-tighter">Section Created!</h3>
                                <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mt-2">Ibigay ang code na ito sa iyong mga estudyante:</p>
                                
                                <div className="mt-6 p-6 bg-indigo-50 rounded-[2rem] border-2 border-dashed border-indigo-200">
                                    <span className="text-4xl font-black text-indigo-600 tracking-widest">{generatedCode}</span>
                                </div>
