...
const SectionForm = ({ courseId, deptId, programId, onNext }) => {
    ...
    const handleSaveSection = async (e) => {
        ...
        try {
            ...
            const response = editingId 
                ? await authAPI.updateSection(editingId, formData, token)
                : await authAPI.createSection(courseId, deptId, programId, formData, token);
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await authAPI.deleteSection(sectionToDelete.id, token);
            if (response.ok) {
                await fetchSections();
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {loading && sections.length === 0 ? (
                <div className="py-24 text-center font-black text-gray-200 italic animate-pulse tracking-widest">LOADING SECTIONS...</div>
                    ...
                    {filteredSections.map((section) => (
                        ...
                        >
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                ...
                                <button onClick={(e) => confirmDelete(e, section)} className="text-[9px] font-black uppercase text-red-500 bg-red-50 px-2 py-1 rounded-lg hover:bg-red-500 hover:text-white transition-all">Del</button>
                            </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md overflow-hidden animate-in zoom-in duration-200 shadow-2xl">
                        {!generatedCode ? (
                            ...
                            <div className="p-10 text-center animate-in fade-in zoom-in duration-300">
                                ...
                                <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mt-2">Share this code with your students:</p>
                    ...
                    <div className="bg-white rounded-[2.5rem] w-full max-w-sm overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <div className="p-8 text-center">
                            <p className="text-gray-500 text-sm font-medium leading-relaxed mb-6">
                                Are you sure you want to delete <span className="font-black text-gray-800">"{sectionToDelete?.section_name}"</span>? 
