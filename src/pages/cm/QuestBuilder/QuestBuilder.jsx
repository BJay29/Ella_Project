      ...
      <div className="w-full font-sans p-4 bg-gray-50 min-h-screen text-slate-900">
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={openCreateModal} className="bg-[#6366F1] hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-md transition-all active:scale-95">
              + Create New Quest
            </button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {quests.map((quest) => (
                <div
                  key={quest.quest_id || quest.id}
                  onClick={() => goToManageLevels(quest)}
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <button
                        onClick={(e) => openAssignModal(e, quest)}
                  ...
                  <div className="flex gap-3 pt-4 border-t border-gray-50">
                    <button onClick={(e) => openEditModal(e, quest)} className="flex-1 bg-[#F3F4F6] hover:bg-gray-200 text-indigo-600 py-3 rounded-xl font-black text-[10px] uppercase transition-all">✏️ Edit</button>
        ...
        {/* ASSIGN MODAL - Pinasa natin ang selectedQuest para makuha ang ID sa loob */}
        <AssignQuestModal
            ...
            <div className="bg-white w-full max-w-md rounded-[24px] shadow-2xl p-8 border border-gray-100 animate-in fade-in zoom-in duration-200">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-1.5">
                  ...
                  <select className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500" value={questData.quest_type} onChange={(e) => setQuestData({...questData, quest_type: e.target.value})} required>
                ...
                <div className="space-y-1.5">
                  ...
                  <select className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500" value={questData.quest_level} onChange={(e) => setQuestData({...questData, quest_level: e.target.value})} required>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={questData.quest_number} onChange={(e) => setQuestData({...questData, quest_number: e.target.value})} required />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Passing Score</label>
                    <input type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-indigo-500" value={questData.passing_score} onChange={(e) => setQuestData({...questData, passing_score: e.target.value})} required />
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-sm rounded-[24px] p-8 text-center animate-in fade-in zoom-in duration-200">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteModal(false)} className="flex-1 px-6 py-3 border border-gray-200 rounded-xl font-bold text-gray-500 text-xs hover:bg-gray-50">Cancel</button>
                <button onClick={confirmDelete} disabled={isSubmitting} className="flex-1 bg-red-500 text-white px-6 py-3 rounded-xl font-bold text-xs shadow-md hover:bg-red-600 disabled:opacity-50">
    ...
    <div className="w-full font-sans p-6 bg-gray-50 min-h-screen">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <button onClick={goBackToList} className="flex items-center gap-2 text-indigo-600 font-black text-[10px] uppercase tracking-widest hover:gap-4 transition-all">
            &larr; Back to Quests
          </button>
        ...
        <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden min-h-[500px] flex flex-col">
          <div className="p-8">
            <div className="flex justify-between items-center mb-8 border-b border-gray-50 pb-6">
              ...
              <button onClick={openLevelCreateModal} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-md transition-all active:scale-95">
                + Create Level
              </button>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-gray-400 text-sm max-w-xs">Start building your quest by adding your first level.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {levels.map((lvl, index) => (
                      <tr key={lvl.quest_level_id || lvl.id || `lvl-${index}`} onClick={() => goToSelection(lvl)} className="group hover:bg-indigo-50/50 transition-all cursor-pointer">
            ...
            <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl p-10 animate-in fade-in zoom-in duration-200">
              ...
              <form onSubmit={handleLevelSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  ...
                  <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500" value={levelData.title} onChange={(e) => setLevelData({...levelData, title: e.target.value})} required />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest ml-1">Order / Number</label>
                  <input type="number" className="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500" value={levelData.level_order} onChange={(e) => setLevelData({...levelData, level_order: e.target.value})} required />
                </div>
          ...
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-sm rounded-[24px] p-8 text-center animate-in fade-in zoom-in duration-200">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={confirmDeleteLevel} disabled={isSubmitting} className="flex-1 bg-red-500 text-white px-6 py-3 rounded-xl font-bold text-xs shadow-md hover:bg-red-600 disabled:opacity-50">
