...
// ── ActivityCreator: saves activity title / difficulty / passing_score only.
const ActivityCreator = ({
  ...
  useEffect(() => {
    ...
    const actId =
      ...
      existingActivity?.id          ||
      existingActivity?._id         || null;

    activityIdRef.current = actId;

    setFormData({
      title:         existingActivity?.title         || '',
      difficulty:    capitalize(existingActivity?.difficulty),
      passing_score: existingActivity?.passing_score != null
        ? String(existingActivity.passing_score)
        : '',
    });
  ...
  const handleSave = async () => {
    ...
    if (!formData.title.trim()) {
      setErrorModal({ show: true, title: 'Missing Info', message: 'Please enter an activity title.' });
      return;
    }
    ...
    try {
      ...
      console.error('ActivityCreator save error:', err);
  ...
  const isEditing = !!activityIdRef.current;
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              ...
              <button
                ...
                className="w-full bg-amber-500 text-white py-5 rounded-[22px] font-black text-[11px] uppercase tracking-widest hover:bg-amber-600 transition-all active:scale-95 disabled:opacity-60"
              >
